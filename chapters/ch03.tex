...
We made fragments of Nups to tether to the hydrogels.  All of the Nups we used came from Nsp1, which is an essential Nup for selective transport (cite something about someone who tried to delete it).  Like all FG Nups, Nsp1 has an ordered domain which anchors it to the channel wall, and a disordered, FG-filled region.  As a whole, this region aggregates in buffer \cite{frey06, frey 07} but not in cells \cite{Loren's paper even though it's technically not the whole FG domain?} where nonspecific interactions stabilize it.  
...
We use two fragments of Nsp1 developed by Loren's old group, FSFG and FG124. Each are 124 amino acids long and contain six transport-factor binding motifs.  FSFG has six FSFG motifs and is designed not to aggregate.  It's entirely disordered, quite stable, expresses well, and can be concentrated up to about 30 mg/mL before crashing out.  FG124 contains six FG motifs but they're not all the same kind (lookup: find sequence).  It aggregates in buffer over the course of several hours. It needs to be kept in 7M guanidine hydrochloride at all times.  It also expresses well.

Both proteins are expressed in Bl21 DE3 Gold cells in the pRSF plasmid, which is Kan resistant.  They are tagged with a his tag on the C-terminus and are usually purifed using a metal affinity column (see appendix).  They last a long time in the fridge but PIC should be added occasionally because they are disordered.  They can be kept in almost any buffer at a wide range of pH (certainly 3-8, maybe more).  We have versions of FSFG with one or two terminal cysteines, a cysteine and a ybbr labeling tag, concat 2 and concat 3 with and without single cysteines, 2-mer and 4-mer (with and without cysteines?).  FG124 we have with and without a single cysteine.  We have full-length Nsp1 and just the FG-domain as well, though I've maybe purified them once.  They don't express nearly as well as the shorter constructs.

I've tested that they look fine on an SDS PAGE gel after lyophilizing and resuspending.  Usually I lyophilize them so I can control the final nominal concentration in the precursor solution.  I let them sit at room temp to resuspend for about 20 minutes.  The cysteines in cys-labeled FSFG form disulfide bonds extremely rapidly after being removed from reducing agents.  Put the Ellman's reagent protocol in the appendix about bis-labeling.  You can't use a Bradford on them because they don't have the right amino acids (?) so use a BCA.

I need a figure for the variants.
