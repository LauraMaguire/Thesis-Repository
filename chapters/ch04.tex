...
% trypsin digestion assay pgs 94-103 LKM book 6
...
%We first tried to use published data for the off-rate constant for NTF2 and both FSFG constructs \cite{hayama18}.  The published data showed $K_D = 545$  $\mu$M for FSFG concat-1; for FSFG concat-2, $K_D = 395$ $\mu$M.  

%Assuming a diffusion-limited on-rate constant of $\kon = 10^3$ $\mu$M$^{-1}$ s$^{-1}$ for both constructs, this corresponds to $\koff = 5.45 \times 10^5$ s$^{-1}$ and $\koff = 3.95 \times 10^5$ s$^{-1}$, respectively.  
...
The diffusion of the nonbinding 30-kDa protein mCherry was used as a proxy for the free diffusion of similarly-sized NTF2 within the FSFG hydrogels.
...
Scripts to perform the following calculations are available at \url{https://github.com/LauraMaguire/image-processing} and \url{https://github.com/LauraMaguire/FRAP-analysis}.
...
% t-test cct1 DB compared to 0: The t-value is 2.586766. The value of p is .025284. The result is significant at p < .05.
% t-test cct2 DB compared to 0: The t-value is 1.766134. The value of p is .127806. The result is not significant at p < .05.
