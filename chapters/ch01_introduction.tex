...
\section{The nuclear pore complex as a selective filter}

The nuclear pore is the filter that regulates all transport between the cell's nucleus and its cytoplasm.  It's a unique filter because it blocks the passage of proteins that are more than about five nanometers across, but allowed selected proteins called transport factors to pass through rapidly.  These transport factors can carry cargo, and the entire complex can be up to 40 nm across and still easily transit the pore.  Therefore, the nuclear pore is a filter that keeps small things out but lets selected large things pass.  We know what components are involved in nuclear transport, but we don't know exactly how it works.
...
They are also known (in human cells?) as importins and exportins, or collectively as the importin $\beta$ superfamily \cite{tu11}.     Most Kaps bind their cargo directly via a nuclear localization signal (NLS, for nuclear import) or nuclear export signal (NES, for nuclear export).  NLS and NES are 5-7 amino acid tags found on cargo \cite{}.  However, Kap95? (importin $\beta$) uses the adaptor protein Kap60? (importin $\alpha$) to bind its cargo. In general, Kaps are on the order of 100 kDa in size, well above the passive permeability limit \cite{}. Kaps may contribute to the selectivity barrier.
...
Aside from the NPC itself, transport factors (TFs) are a class of proteins essential for selective transport.  The energetic cost of selectivity is captured in the Ran GTP/GDP cycle.
%
...
\begin{SCfigure}
\centering
\includegraphics[width=0.5\linewidth]{figs/ch01/elbaum}
...
\section{Goals/motivation of this project}
...
\subsection{Model transport as simply as possible}

What are the minimum requirements for selective transport?
...
\subsection{Experimentally test model using bulk-material NPC mimics}

Can we create a bulk medium that has the same selectivity properties as the pore?
Predict the selectivity of the bulk medium based on our model of transport
Measure bound diffusion constant in NPC mimic

PARP1 diffusion
Simulate hopping of PARP1 using parameters from Luger lab
Bound diffusion constants
Incorporate binding into diffusion model
Fit data that I’ve already taken
Test effect of changing tether length
Dextran/dextranase porogen system
Test effect of lower-MW dextran
Decide whether it’s likely to be paper-worthy
If so, test protein entry into gel at a number of dextran concentrations
